...
module App =
    ...
    let html container =
        ...
        session (function
            | UserLoggedOn { Username = username } ->
                let items = Db.getCartDetails username ctx |> List.sumBy (fun c -> c.Count)
                result items (Some username)
            | CartIdOnly cartId ->
                let items = Db.getCartDetails cartId ctx |> List.sumBy (fun c -> c.Count)
                result items None
            | _ -> result 0 None)
    ...
    let register =
        choose [
            ...
            POST >>= bindToForm Form.register (fun form ->
                ...
                | Some existing ->
                    View.register "Sorry this username is already taken. Try anohter one." |> html
                | None ->
                    let (Password password) = form.Password
        ...
        session (function
            ...
            | UserLoggedOn { Username = cartId } | CartIdOnly cartId ->
                ...
                | Some cart ->
                    ...
                    Db.getCartDetails cartId ctx |> View.cart |> Html.flatten |> Html.xmlToString |> OK
                | None ->
                    never)
    ...
    let webPart =
        choose [
            ...
            pathRegex "(.*)\.(css|js|png|gif)" >>= Files.browseHome
...
//    let _, server = startWebServerAsync defaultConfig webPart
//    Async.Start(server)
