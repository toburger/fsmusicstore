              ...
              [ { Legend = "Album"
                  Fields =
                      [ { Label = "Genre"
                        ...
                        { Label = "Album Art Url"
                          Xml = input (fun f -> <@ f.ArtUrl @>) [ "value", album.AlbumArtUrl ] } ] } ]
...
let emptyCart = [
    ...
    aHref Path.Store.overview (text "store")
    text "!"
]
...
let nonEmptyCart (carts : Db.CartDetails list) = [
    ...
    table [
        ...
        for cart in carts ->
            tr [
                ...
                td [
                    aHrefAttr "#" [ "class", "removeFromCart"; "data-id", string cart.AlbumId ] (text "Remove from cart")
                ]
            ]
        yield tr [
    ...
    scriptAttr [ "type", "text/javascript"; " src", "//code.jquery.com/jquery-1.11.3.min.js" ] [ text "" ]
